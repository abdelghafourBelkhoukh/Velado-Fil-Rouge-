<template>
    <div>
        <div class="flex justify-center px-6 py-8">
            <div class="text-red-700 text-2xl text-center font-bold">Orders History</div>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table text-grey-darkest text-center w-full h-full">
                        ...
                        <tr v-for="item in OrderData" class="border-b">
                            ...
                            <td class="py-4 text-green-600 font-medium" v-if="item.confirmationOrder=='Accepted'">{{item.status}}</td>
                            <td class="py-4 text-red-600 font-medium" v-if="item.confirmationOrder=='refused'">{{item.confirmationOrder}}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
export default{
    ...
    computed:{
        OrderData(){
            let data = this.$store.state.historyData;
            let newData = [];
            let result = [];
